...
def main():
  ...
  use_llvm_tools = sys.argv[6] == 'true'
  ...
  for cpu in cpus:
    ...
    if cpu == target_cpu:
      llvm_tool_path = os.path.join(visual_studio_path, 'VC', 'Tools', 'Llvm', cpu, 'bin')
      if use_llvm_tools:
        if os.path.exists(os.path.join(llvm_tool_path, 'clang-cl.exe')):
          vc_bin_dir = llvm_tool_path
      else:
